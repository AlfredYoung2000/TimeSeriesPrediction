...
def plot_pred_test(pred, actual, path, feature, model):
    ...
    plt.legend(loc='best')
    plt.grid()
    plt.title('{}EnergyPredictionUsing {}'.format(feature, model.__class__.__name__), fontsize=18)
    plt.savefig(os.path.join(path, '{}EnergyPredictionUsing{}.png'.format(feature, model.__class__.__name__)))
